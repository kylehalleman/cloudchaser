...
Class MbqRdEtForumPost extends MbqBaseRdEtForumPost {
    ...
    public function processContentForDisplay($content, $returnHtml) {
        ...
        */
    	...
    	$post = preg_replace_callback('/\[url=\/mobiquo\.php(\?p\=.*?)\](.*?)\[\/url\]/i', create_function('$matches','return "[url=".MbqMain::$oMbqAppEnv->rootUrl."index.php".$matches[1]."]".$matches[2]."[/img]";'), $post);
